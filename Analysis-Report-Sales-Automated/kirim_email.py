...
def buat_body_html(ringkasan, awal, akhir, periode_label):
    ...
    total_pendapatan = format_usd(ringkasan.get("total_pendapatan", 0))
    total_transaksi  = f"{ringkasan.get('total_transaksi', 0):,}"
    rata_harian      = format_usd(ringkasan.get("rata_rata_harian", 0))

    hari_terlaris_str = ""
    if ringkasan.get("hari_terlaris"):
        import pandas as pd
        ht = ringkasan["hari_terlaris"]
        hari_terlaris_str = f"{pd.to_datetime(ht['tanggal']).strftime('%B %d, %Y')} ({format_usd(ht['total'])})"
    ...
    if ringkasan.get("per_produk") is not None:
        for i, row in ringkasan["per_produk"].head(5).iterrows():
            ...
            <tr style="background:{'#f8fafc' if rank % 2 == 0 else 'white'}">
                ...
                <td style="padding:8px 12px;border-bottom:1px solid #e2e8f0;text-align:right">{format_usd(row['total_pendapatan'])}</td>
            </tr>"""
      ...
      <div style="max-width:600px;margin:24px auto;background:white;border-radius:12px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.1)">
        ...
        <div style="background:#1a3c5e;padding:28px 32px;text-align:center">
          ...
          <p style="margin:6px 0 0;color:#a0bcd8;font-size:13px">
            Monthly Sales Report — {periode_label}
          </p>
        </div>
        ...
        <div style="padding:24px 32px 16px">
          <p style="margin:0;color:#4a5568;font-size:14px;line-height:1.6">
            ...
            Please find below the sales report for the period
            <strong>{awal.strftime('%B %d, %Y')}</strong> to
            <strong>{akhir.strftime('%B %d, %Y')}</strong>.
            The full report is available as a PDF attachment.
          </p>
        </div>
            ...
            <tr>
              <td width="50%" style="vertical-align:top">
                <div style="background:#f0f4f8;border-left:4px solid #2e86de;border-radius:6px;padding:14px 16px">
                  <div style="color:#718096;font-size:11px;text-transform:uppercase;letter-spacing:0.5px">Total Revenue</div>
              ...
              <td width="50%" style="vertical-align:top">
                <div style="background:#f0f4f8;border-left:4px solid #27ae60;border-radius:6px;padding:14px 16px">
                  <div style="color:#718096;font-size:11px;text-transform:uppercase;letter-spacing:0.5px">Total Transactions</div>
            ...
            <tr>
              <td style="vertical-align:top;padding-top:8px">
                <div style="background:#f0f4f8;border-left:4px solid #f39c12;border-radius:6px;padding:14px 16px">
                  <div style="color:#718096;font-size:11px;text-transform:uppercase;letter-spacing:0.5px">Daily Average</div>
              ...
              <td style="vertical-align:top;padding-top:8px">
                <div style="background:#f0f4f8;border-left:4px solid #e74c3c;border-radius:6px;padding:14px 16px">
                  <div style="color:#718096;font-size:11px;text-transform:uppercase;letter-spacing:0.5px">Best-Selling Product</div>
        ...
        {"<!-- Best Day -->" + f'<div style="padding:0 32px 16px"><p style="margin:0;background:#fffbeb;border:1px solid #f6d860;border-radius:6px;padding:12px 16px;color:#744210;font-size:13px">📅 <strong>Best day:</strong> ' + hari_terlaris_str + '</p></div>' if hari_terlaris_str else ""}

        <!-- Top 5 Products -->
        <div style="padding:0 32px 24px">
          <h3 style="margin:0 0 12px;color:#1a3c5e;font-size:14px">🏆 Top 5 Products</h3>
          <table width="100%" cellspacing="0" cellpadding="0" style="border-collapse:collapse;border:1px solid #e2e8f0;border-radius:8px;overflow:hidden">
            <thead>
              <tr style="background:#1a3c5e">
                ...
                <th style="padding:10px 12px;color:white;font-size:12px;text-align:left">Product</th>
                <th style="padding:10px 12px;color:white;font-size:12px;text-align:right">Qty</th>
                <th style="padding:10px 12px;color:white;font-size:12px;text-align:right">Revenue</th>
              </tr>
            </thead>
        ...
        <div style="padding:0 32px 24px">
          <div style="background:#ebf8ff;border:1px solid #bee3f8;border-radius:6px;padding:12px 16px">
            <p style="margin:0;color:#2c5282;font-size:13px">
              📎 The full report including charts and transaction details is available in the attached PDF.
            </p>
          </div>
        </div>
        ...
        <div style="background:#f7fafc;padding:16px 32px;border-top:1px solid #e2e8f0;text-align:center">
          <p style="margin:0;color:#a0aec0;font-size:11px">
            This email was sent automatically by the {PERUSAHAAN['nama']} reporting system<br>
...
def kirim_email(pdf_path, ringkasan, awal, akhir):
    ...
    msg["Subject"] = f"Monthly Sales Report — {periode_label} | {PERUSAHAAN['nama']}"
